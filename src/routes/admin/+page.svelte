<script>
    ...
    function makeLink() {
        const url = window.prompt('URL do link (http(s)://, mailto:, tel:, ou caminho relativo):', 'https://');
            ...
            <textarea
                ...
                on:keydown={(e) => {
                    ...
                    if (mod && !e.altKey && !e.shiftKey) {
                        if (k === 'b') { e.preventDefault(); makeBold(); }
                        else if (k === 'i') { e.preventDefault(); makeItalic(); }
                        else if (k === 'k') { e.preventDefault(); makeLink(); }
                        else if (k === '0') { e.preventDefault(); clearFormatting(); }
                    }
                }}
            ></textarea>
