<script>
	...
	const defaultDescription =
		'Fabio Sales is a design leader and product builder with over 30 years of experience turning complex information and systems into clear, scalable experiences, combining visual craft, product thinking, technology, and business leadership.';
	...
	let jsonLd = null;
	{
		try {
			...
			for (const o of objs) {
				const bloco = o.raw;
				if (!introConf && bloco && typeof bloco === 'object' && bloco.intro) introConf = bloco.intro;
				if (!lcpFound && typeof bloco === 'object' && (bloco.nome || (bloco.imagem && bloco.imagem.nome))) {
					...
					if (img && img.nome) {
						...
						} else lcpHref = withBase(`/imgs/${img.nome}`, base);
						lcpImagesrcset = '';
			...
			jsonLd = {
				...
				image: imageUrl,
				sameAs: [],
				jobTitle: 'Design Leader and Product Builder',
	...
	function setupIntro(el) {
		...
		runWhenIdle(() => {
			...
			el.querySelectorAll('.inline-logo').forEach((node) => node.style.setProperty('--inline-logo-url', `url('${logoSrc}?v=${logoTick}')`));
			wrapWords(el);
	...
	onMount(() => {
		...
		function tryInitOnce() {
			...
			if (!candidates.length) candidates = Array.from(container.querySelectorAll('p')).filter((el) => (el.textContent || '').trim().length > 20).slice(0, 2);
		...
		if (!tryInitOnce()) {
			...
			const mo = new MutationObserver(() => {
				if (tryInitOnce()) return mo.disconnect();
...
<svelte:head>
	...
	{/if}
	{#if jsonLd}<script type="application/ld+json">{JSON.stringify(jsonLd)}</script>{/if}
	{#if lcpHref}
