<script>
    ...
    onMount(() => {
        if (typeof window !== 'undefined' && 'IntersectionObserver' in window) {
                ...
                for (const e of entries) {
                    if (e.isIntersecting) {
                        try {
                            ...
                            preloader.onloadeddata = () => { preloaded = true; };
            ...
            try {
                preloader = document.createElement('video');
                preloader.preload = 'metadata';
                if (Array.isArray(sources) && sources.length) preloader.src = sources[0]; else preloader.src = src;
                preloader.onloadeddata = () => { preloaded = true; };
                preloader.onerror = () => { preloaded = true; };
            } catch (e) { preloaded = false; }
            currentSrc = src;
    ...
    onDestroy(() => {
        ...
        if (preloader) try { preloader.src = ''; } catch (e) {}
    ...
    $: {
        ...
        if (_attachedCleanup) {
            try { _attachedCleanup(); } catch (e) { /* ignore */ }
            _attachedCleanup = null;
        }
        if (videoEl) {
            const _onplay = () => { isPlaying = true; };
            const _onpause = () => { isPlaying = false; };
            const _onended = () => { isPlaying = false; };
            try {
    ...
    function togglePlay() {
        ...
        if (isPlaying) {
            ...
            try {
                videoEl.pause();
            } catch (e) { console.debug('pause fail', e); }
            // ensure state reflects the element
            isPlaying = !videoEl.paused;
        } else {
            // user wants to play — clear userPaused
            userPaused = false;
            videoEl.play().then(() => { isPlaying = !videoEl.paused; }).catch((err) => { console.debug('play failed', err); isPlaying = false; });
        }
    }
    ...
    function toggleMute() {
        ...
        if (!videoEl) return;
        isMuted = !isMuted;
        try { videoEl.muted = isMuted; } catch (e) { console.debug('set muted failed', e); }
        if (!isMuted) {
            // ensure playing with sound
            videoEl.play().catch((e) => { console.debug('play after unmute failed', e); });
        }
    }
    ...
    function showControlsTemporary() {
        ...
        if (_controlsHideTimer) clearTimeout(_controlsHideTimer);
        _controlsHideTimer = setTimeout(() => { controlsVisible = false; _controlsHideTimer = null; }, 3000);
    }
...
<figure bind:this={wrapperEl} class={`video-block video-block-wrapper ${sizeClass}`} class:show={show}>
    <div class="video-inner">
        ...
        <div class="video-controls" aria-hidden={!show} class:visible={controlsVisible} on:touchstart={onTouchStart} on:pointerdown={onPointerDown}>
            <Button on:click={() => { console.debug('play clicked'); togglePlay(); }} variant={isPlaying ? 'primary' : 'default'} active={isPlaying}>{isPlaying ? 'Pause' : 'Play'}</Button>
            <Button on:click={() => { console.debug('mute clicked'); toggleMute(); }} variant={isMuted ? 'default' : 'primary'} active={!isMuted}>{isMuted ? 'Unmute' : 'Mute'}</Button>
        </div>
        {#if error}
...
<style>
    .video-block { margin: 6em auto 1rem auto; text-align:center; max-width:100%; display:flex; flex-direction:column; align-items:center; }
    ...
    .video-error { position: absolute; left: 12px; bottom: 12px; background: rgba(180,0,0,0.85); color: #fff; padding: 6px 10px; border-radius: 6px; z-index: 11; }
    @keyframes shimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }
    .video-block figcaption { font-size:0.95em; color:#555; margin-top:0.5em; }

    .video-block-wrapper.video-pp { max-width: 250px; }
    .video-block-wrapper.video-p  { max-width: 500px; }
    .video-block-wrapper.video-m  { max-width: 620px; }
    .video-block-wrapper.video-g  { max-width: 860px; }

    @media (max-width:600px) {
        .video-block-wrapper.video-pp, .video-block-wrapper.video-p, .video-block-wrapper.video-m, .video-block-wrapper.video-g { width: calc(100% - (var(--grid) * 4)); box-sizing:border-box; }
    }
</style>
