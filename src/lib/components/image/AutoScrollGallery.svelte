<script>
	...
	function endDrag(e) {
		...
		if (containerEl && dragPointerId !== null) {
			try {
				containerEl.releasePointerCapture && containerEl.releasePointerCapture(dragPointerId);
			} catch {}
		}
		isDragging = false;
		dragPointerId = null;
	...
	function step() {
		...
		if ((containerEl.scrollLeft || 0) >= maxScroll - 1) {
			try {
				containerEl.scrollTo({ left: 0, behavior: 'smooth' });
			} catch {
				containerEl.scrollLeft = 0;
	...
	function computeScale() {
		...
		const maxH = Math.max(
			0,
			...items.filter((it) => it.type === 'image' && it.height).map((it) => Number(it.height) || 0)
		);
		maxImageHeight = maxH;
		if (!maxH) {
			sizeScale = 1;
...
<style>
	...
	.scroll-viewport.size-PP {
		max-width: 250px;
	}
	.scroll-viewport.size-P {
		max-width: 500px;
	}
	.scroll-viewport.size-M {
		max-width: 620px;
	}
	.scroll-viewport.size-G {
		max-width: 860px;
	}
	...
	@media (max-width: 600px) {
		/* Mobile: allow swipe + snap to center */
		.scroll-viewport {
			overflow-x: auto;
